...
var commandsPool = [];
const getAverage = ()=>{
    commandsPool = commandsPool.slice(commandsPool.length - 20, commandsPool.length);
...
window.onload = function() {
    ...
    if (isMobile()) {
        window.addEventListener("resize", (e)=>{
            ...
            if (myElement === document.activeElement) //myElement Has Focus
            console.log("KEYBOARD_OPEN");
            else console.log("KEYBOARD_CLOSE");
        });
        function handleOrientation(e) {
            if (ready) {
                ...
                if (direcionalRight) {
                    if (humanForceRight > 0) direcionalLeft.style.opacity = 0.3;
                    else {
                        let leftCalc = Math.abs(humanForceLeft / 100);
                        direcionalLeft.style.opacity = leftCalc > 0.3 ? leftCalc : 0.3;
                    }
                    if (humanForceLeft > 0) direcionalRight.style.opacity = 0.3;
                    else {
                ...
                playerHuman = setTimeout(()=>{
                    ...
                    if (hasDebug && document.getElementById("footer-debug")) document.getElementById("footer-debug").innerHTML = `forceLeft = ${humanForceLeft} <br> forceRight = ${humanForceRight}`;
                }, 10);
...
//# sourceMappingURL=index.4655a0b2.js.map
